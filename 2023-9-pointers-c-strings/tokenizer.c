...
not_a_number:
  ...
  {
    for (int i = 0; input[i] != '\0'; i++) // 处理不合法的变量
      if ((input[0] >= '0' && input[0] <= '9') || ((input[i] != '_') && ((input[i] < '0') || (input[i] > '9' && input[i] < 'A') || (input[i] > 'Z' && input[i] < 'a') || (input[i] > 'z'))))
      {
        flag1 = 1;
        return 0;
      }
...
int judge(char input[]) // eliminate illegal input
{
  for (int i = 0; input[i] != '\0'; i++)
    if ((input[i] == '|' && input[i + 1] == '|')                                                         // 约定中没有 || 运算符
        || (input[i] == '-' && ((input[i + 1] >= '0' && input[i + 1] <= '9') || (input[i + 1] == '.')))) // 约定中整数或浮点数不含负号
    {
      flag1 = 1;
      return 0;
    }

  int count = 0; // 处理含多个'.'的浮点数
  for (int i = 0; input[i] != '\0'; i++)
  {
    if (input[i] == '.' || (input[i] >= '0' && input[i] <= '9'))
      ;
